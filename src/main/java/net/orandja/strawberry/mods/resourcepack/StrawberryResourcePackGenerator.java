...
public abstract class StrawberryResourcePackGenerator {
    ...
    public static void save() {
        ...
        itemCustomModelData.forEach((path, customModels) -> {
            JsonObject modelObject = getSourceModel(path);
            ...
            for (CustomModelData modelData : customModels) {
                ...
                override.addProperty("model", "minecraft:"+ modelData.model);
                overrides.add(override);

                JsonObject customItemModel = new JsonObject();
                customItemModel.addProperty("parent", "minecraft:item/generated");
                JsonObject textures = new JsonObject();
                textures.addProperty("layer0", modelData.texture);
                customItemModel.add("textures", textures);

                writeTo("../resourcepack/assets/minecraft/models/" + modelData.model + ".json", customItemModel);
            }

            writeTo("../resourcepack/assets/minecraft/models/" + path + ".json", modelObject);
        });
    }

...
//        if(!world.isClient && world instanceof ServerWorld serverWorld && BlockUtils.isWood(state) && PlayerUtils.areBothToolsSuitable(player, state, AxeItem.class)) {
//            player.incrementStat(Stats.MINED.getOrCreateStat(block));
//            player.addExhaustion(0.005f);
//
//            List<ItemStack> stacksToDrop = new ArrayList<>();
//            Map<Block, AtomicInteger> blocksCount = new HashMap<>();
////            AtomicInteger count = new AtomicInteger();
//            ToolTask.TASK_AXE.getExecutor().execute(world, pos, player, state, PlayerUtils.getBothTools(player), it -> {
//                player.getMainHandStack().damage(1, player, e -> e.sendEquipmentBreakStatus(EquipmentSlot.MAINHAND));
//                player.getOffHandStack().damage(1, player, e -> e.sendEquipmentBreakStatus(EquipmentSlot.OFFHAND));
//
//                BlockState otherState = world.getBlockState(it);
//
//                player.incrementStat(Stats.MINED.getOrCreateStat(otherState.getBlock()));
//                player.addExhaustion(0.005f);
//                world.setBlockState(it, Blocks.AIR.getDefaultState(), 2);
//
//                Block.getDroppedStacks(otherState, serverWorld, it, world.getBlockEntity(it), player, stack).forEach(droppedStack -> {
//                    stacksToDrop.stream().filter(stackToDrop -> StackUtils.canMerge(stackToDrop, droppedStack)).findFirst().ifPresentOrElse(stackToDrop -> {
//                        stackToDrop.increment(droppedStack.getCount());
//                    }, () -> {
//                        stacksToDrop.add(droppedStack);
//                    });
//                });
//
////                blocksCount.computeIfAbsent(state.getBlock(), ignored ->  new AtomicInteger());
////                if(blocksCount.get(state.getBlock()).incrementAndGet() >= 64) {
////
...
//            });
//
//            Block.getDroppedStacks(state, serverWorld, pos, blockEntity, player, stack).forEach(droppedStack -> {
//                stacksToDrop.stream().filter(stackToDrop -> StackUtils.canMerge(stackToDrop, droppedStack)).findFirst().ifPresentOrElse(stackToDrop -> {
//                    stackToDrop.increment(droppedStack.getCount());
//                }, () -> {
//                    stacksToDrop.add(droppedStack);

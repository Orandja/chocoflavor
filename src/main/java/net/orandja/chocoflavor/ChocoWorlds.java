...
public class ChocoWorlds {
    private ChocoWorlds() {}


    // returns true if the player can destroy the block
    public static final List<GlobalUtils.TriPredicate<World, BlockPos, Entity>> ENTITY_CAN_DESTROY = Lists.newArrayList();
    // returns true if the explosion can destroy the block
    public static final List<BiPredicate<World, BlockPos>> EXPLOSION_CAN_DESTROY = Lists.newArrayList();
    // returns false if the hopper can extract item from block **Might be changed in the future
    public static final List<BiPredicate<World, BlockPos>> HOPPER_CANNOT_EXTRACT = Lists.newArrayList();

    public static void preventsExtraction(World world, Object maybeHopper, CallbackInfoReturnable<Boolean> info) {
        if(maybeHopper instanceof HopperBlockEntity hopper && hopper instanceof Handler hopperProtect && hopperProtect.preventsExtract(world, hopper.getHopperX(), hopper.getHopperY() + 1.0, hopper.getHopperZ())) {
